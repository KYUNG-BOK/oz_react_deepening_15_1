...
import useBoardStore from '../store'; // zustand store 불러오기

const BoardConfirmModal = ({ onClose, id }) => {
  const removeBoard = useBoardStore((state) => state.removeBoard); // 삭제 메서드 추출

  const handleDelete = () => {
    removeBoard(id); // zustand 상태에서 해당 보드 삭제
    onClose(); // 모달 닫기
  };
  ...
  return (
    <div
      onClick={onClose}
      className="fixed inset-0 flex items-center justify-center z-51 bg-black/70"
    >
      <div
